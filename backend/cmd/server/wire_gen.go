...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	openAIOAuthService := service.ProvideOpenAIOAuthService(accountRepository, proxyRepository, openAIOAuthClient, privacyClientFactory, settingService)
	...
	accountExportConfirmationLimiter := repository.NewAccountExportConfirmationLimiter(redisClient)
	accountHandler := admin.ProvideAccountHandler(configConfig, adminService, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, grokOAuthService, rateLimitService, accountUsageService, accountTestService, concurrencyService, crsSyncService, sessionLimitCache, rpmCache, compositeTokenCacheInvalidator, grokQuotaService, totpService, settingService, auditLogService, accountExportConfirmationLimiter)
	...
	imageStorageSettingService := service.ProvideImageStorageSettingService(settingRepository, secretEncryptor, backupService, imageStorageFactory, invalidationBus, configConfig)
	...
	settingHandler := handler.ProvideAdminSettingHandler(settingService, emailService, turnstileService, aliyunCaptchaService, opsService, paymentConfigService, paymentService, userAttributeService, notificationEmailService, totpService, userService, openAICodexVersionSyncService)
	opsHandler := handler.ProvideOpsHandler(opsService, ipAccessControlService)
	...
	clientDisconnectRiskRepository := repository.NewClientDisconnectRiskRepository(db)
	clientDisconnectRiskService := service.NewClientDisconnectRiskService(clientDisconnectRiskRepository, settingService, apiKeyAuthCacheInvalidator)
	adminUsageHandler := handler.ProvideAdminUsageHandler(usageService, apiKeyService, adminService, usageCleanupService, clientDisconnectRiskService)
	...
	contentModerationService := service.ProvideContentModerationService(settingRepository, contentModerationRepository, contentModerationHashCache, groupRepository, userRepository, proxyRepository, apiKeyAuthCacheInvalidator, emailService, apiKeyRepository)
	...
	imageTaskService := service.ProvideImageTaskService(imageTaskStore, imageTaskHistoryRepository, imageStorageSettingService)
	asyncImageRateLimitStore := repository.NewAsyncImageRateLimitStore(redisClient)
	asyncImageRateLimiter := service.NewAsyncImageRateLimiter(asyncImageRateLimitStore, settingService)
	asyncImageHandler := handler.ProvideAsyncImageHandler(imageTaskService, openAIGatewayHandler, asyncImageRateLimiter, opsService)
	...
	ipAccessControlMiddleware, err := middleware.ProvideIPAccessControlMiddleware(ipAccessControlService, configConfig)
	if err != nil {
		return nil, err
	}
	...
	openAIGroupQuotaFollowResetService := service.ProvideOpenAIGroupQuotaFollowResetService(openAIGroupQuotaFollowResetRepository, billingCacheService, openAIQuotaService)
