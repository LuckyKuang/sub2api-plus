...
func registerAccountRoutes(admin *gin.RouterGroup, h *handler.Handlers, stepUpAuth middleware.StepUpAuthMiddleware) {
	accounts := admin.Group("/accounts")
	{
		...
		accounts.GET("/:id/usage-alert", h.Admin.Account.GetUsageAlert)
		accounts.PUT("/:id/usage-alert", h.Admin.Account.UpdateUsageAlert)
		accounts.POST("/:id/usage-alert/test", h.Admin.Account.TestUsageAlert)
		accounts.GET("/:id/wecom-usage-alert", h.Admin.Account.GetWeComUsageAlert)
		accounts.PUT("/:id/wecom-usage-alert", h.Admin.Account.UpdateWeComUsageAlert)
		accounts.POST("/:id/wecom-usage-alert/test", h.Admin.Account.TestWeComUsageAlert)
		...
		// 明文账号导出必须由处理器在本次请求中完成密码、必要时 TOTP 的顺序确认。
		accounts.GET("/export/requirements", h.Admin.Account.GetExportRequirements)
		accounts.POST("/export", h.Admin.Account.ExportData)
...
func registerSettingsRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	adminSettings := admin.Group("/settings")
	{
		...
		adminSettings.POST("/openai-codex-version/sync", h.Admin.Setting.SyncOpenAICodexVersion)
		adminSettings.GET("/outbound-identity", h.Admin.Setting.GetOutboundIdentity)
		adminSettings.PUT("/outbound-identity", h.Admin.Setting.UpdateOutboundIdentity)
		adminSettings.POST("/outbound-identity/preview", h.Admin.Setting.PreviewOutboundIdentity)
