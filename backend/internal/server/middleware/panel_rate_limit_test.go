...
func TestPanelRateLimiterPublicIPUsesVerifiedGlobalIdentity(t *testing.T) {
	allower := &fakePanelAllower{}
	p := &PanelRateLimiter{
		limiter:        allower,
		settingService: newPanelRateLimitTestService(t, `{"enabled":true,"user_rpm":0,"heavy_rpm":0,"exempt_admin":true,"public_ip_rpm":1}`),
	...
	router := gin.New()
	router.Use(func(c *gin.Context) {
		// The remote peer is a private reverse proxy. Only the identity written
		// by the global access-control middleware may determine the rate key.
		ip.SetClientIdentity(c, ip.ClientIdentity{
			EffectiveIP:        "203.0.113.9",
			DirectPeerIP:       "10.0.0.8",
			Source:             ip.ClientIdentitySourceTrustedForwarded,
	...
	first.RemoteAddr = "10.0.0.8:1000"
	...
	second.RemoteAddr = "10.0.0.8:1000"
	...
	require.NotContains(t, allower.counts, "panel:public:ip:198.51.100.8")
}

func TestPanelRateLimiterPublicIPSkipsUnsafeGlobalIdentity(t *testing.T) {
	allower := &fakePanelAllower{}
	p := &PanelRateLimiter{
		limiter:        allower,
		settingService: newPanelRateLimitTestService(t, `{"enabled":true,"user_rpm":0,"heavy_rpm":0,"exempt_admin":true,"public_ip_rpm":1}`),
	...
	router.Use(func(c *gin.Context) {
		ip.SetClientIdentity(c, ip.ClientIdentity{
			EffectiveIP:   "203.0.113.9",
			DirectPeerIP:  "10.0.0.8",
			FailureReason: "forwarded_chain_not_sanitized",
		})
		c.Next()
	})
	router.Use(p.PublicIP())
