...
// PublicIP 无需认证的公开接口按客户端 IP 限流。只使用全局 IP 访问控制已
// 验证的身份，不能使用兼容模式下的原始转发头；否则直连源站的客户端可以
// 伪造 X-Forwarded-For 等头部拆分限流桶。解析结果为回环/内网/链路本地地址
// 时跳过计数（这类地址通常是反代内部转发地址，按它计数会把整条反代链路的
// 所有真实用户合并进同一个桶造成大面积误拦截）。
func (p *PanelRateLimiter) PublicIP() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		identity := TrustedClientIdentity(c)
