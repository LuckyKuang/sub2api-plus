...
func (s *OpenAIGatewayService) handleFailoverErrorResponsePassthrough(
	...
	if connectivityErr := s.handleOpenAIUpstreamConnectivityHTTPError(c, account, resp, body, upstreamMsg, upstreamDetail, true); connectivityErr != nil {
...
func (s *OpenAIGatewayService) handleErrorResponsePassthrough(
	...
	setOpsUpstreamError(c, resp.StatusCode, upstreamMsg, upstreamDetail)
	logOpenAIInstructionsRequiredDebug(ctx, c, account, resp.StatusCode, upstreamMsg, requestBody, body)
	if connectivity := classifyOpenAIUpstreamConnectivityFailure(resp.StatusCode, upstreamMsg, body); connectivity != nil {
		...
		if connectivity.TransportFailure != "" {
		...
		return fmt.Errorf("upstream connectivity error: %d", resp.StatusCode)
	}
...
func (s *OpenAIGatewayService) handleStreamingResponsePassthrough(
	...
	noteTurnStateCommitted := func() {
		if turnStateProvenanceNoted || extractOpenAICodexTurnState(resp.Header) == "" {
			return
		}
	...
	for documentScanner.Scan() {
		...
		if data, ok := extractOpenAISSEDataLine(line); ok {
			...
			// non-retryable error frames that should flush to the client.
			lineStartsClientOutput = forceFlushFailedEvent || observation.MeaningfulOutput || openAIStreamEventIsTerminal(trimmedData)
			if !lineStartsClientOutput && openAIStreamDataStartsClientOutput(trimmedData, eventType) {
				// Only treat as client output when the official classifier is more
				// conservative than preamble-only: non-retryable error frames.
				if eventType == "error" {
	...
	if !sawDone && !sawTerminalEvent && ctx.Err() == nil {
