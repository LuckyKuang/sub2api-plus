...
func (s *SettingService) buildSystemSettingsUpdates(ctx context.Context, settings *SystemSettings) (map[string]string, error) {
	...
	normalizedOpenAICodexUserAgent, err := NormalizeOpenAICodexUserAgentWithCompatibility(settings.OpenAICodexUserAgent, settings.CodexLegacyClientProfileCompatibilityEnabled)
	if err != nil {
		return nil, err
	}
	...
	if settings.AsyncImageUserImagesPerMinute < 0 {
		settings.AsyncImageUserImagesPerMinute = 0
	}
	updates[SettingKeyAsyncImageUserImagesPerMinute] = strconv.Itoa(settings.AsyncImageUserImagesPerMinute)
