...
func (c stubPricingRemoteClient) FetchPricingJSON(context.Context, string, int64) ([]byte, error) {
...
func TestPricingHotReload_DownloadRefreshesFingerprint(t *testing.T) {
	svc := newHotReloadPricingService(t, `{`+hotReloadModelJSON("custom-a", 4e-6, 8e-6)+`}`, "")
	const version = "v0.0.1+custom.001"
	const pricingURL = "https://pricing.example.test/releases/download/v0.0.1+custom.001/model-pricing.json"
	svc.cfg.Security.URLAllowlist.Enabled = true
	svc.cfg.Security.URLAllowlist.PricingHosts = []string{"pricing.example.test"}
	...
	digest := sha256.Sum256([]byte(hotReloadCatalogJSON))
	require.NoError(t, svc.downloadReleasePricingData(releasePricingManifest{
		manifest: pricingmanifest.Manifest{
			Version:    version,
			PricingURL: pricingURL,
			SHA256:     hex.EncodeToString(digest[:]),
		},
		raw: []byte(`{"version":"v0.0.1+custom.001"}`),
	}))
