...
// Keep both exported defaults derived from codexCLIVersion so fallback callers
...
func (s *SettingService) IsOpenAICodexLocalGroupQuotaEnabled(ctx context.Context) bool {
	...
	if cached, ok := s.openAICodexLocalQuotaCache.Load().(*cachedOpenAICodexLocalGroupQuota); ok && cached != nil && time.Now().UnixNano() < cached.expiresAt {
	...
	result, _, _ := s.openAICodexLocalQuotaSF.Do("openai_codex_local_group_quota", func() (any, error) {
		...
		if err != nil && !errors.Is(err, ErrSettingNotFound) {
			...
			entry := &cachedOpenAICodexLocalGroupQuota{expiresAt: time.Now().Add(openAICodexLocalGroupQuotaErrorTTL).UnixNano()}
