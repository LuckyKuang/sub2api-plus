...
func TestCreateUpstreamLiveCallPreservesSession(t *testing.T) {
	...
	account := &Account{
		...
		Credentials: map[string]any{
			...
			"user_agent":         "codex-tui/0.144.6 (Ubuntu 22.4.0; x86_64) xterm-256color (codex-tui; 0.144.6)",
		},
	}
	...
	require.Equal(t, "codex-tui/0.144.6 (Ubuntu 22.4.0; x86_64) xterm-256color (codex-tui; 0.144.6)", upstream.request.Header.Get("User-Agent"))
	require.Equal(t, "codex-tui", upstream.request.Header.Get("Originator"))
	require.Equal(t, "0.144.6", upstream.request.Header.Get("Version"))
