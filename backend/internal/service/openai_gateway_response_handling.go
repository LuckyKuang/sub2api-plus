...
func (s *OpenAIGatewayService) handleStreamingResponseWithReasoning(ctx context.Context, resp *http.Response, c *gin.Context, account *Account, startTime time.Time, originalModel, mappedModel, reasoningEffort string) (*openaiStreamingResult, error) {
	...
	processSSELine := func(line string, queueDrained bool) {
		...
		if data, ok := extractOpenAISSEDataLine(line); ok {
			...
			// A retryable error frame can precede response.failed. Keep it buffered
			...
			if !clientDisconnected {
				...
				if (firstMeaningfulOutput && observation.MeaningfulOutput) || (firstTokenMs == nil && startsVisibleOutput) {
	...
	for {
		...
		case <-firstOutputCh:
			if timing.firstOutputMs != nil || firstOutputProgressObserved {
				stopFirstOutputTimer()
				continue
			}
			_ = resp.Body.Close()
...
func (s *OpenAIGatewayService) bindHTTPResponseAccount(ctx context.Context, c *gin.Context, account *Account, responseID string) {
	...
	logOpenAIWSBindResponseAccountWarn(groupID, account.ID, responseID, s.bindOpenAIResponseAccount(ctx, store, groupID, account, responseID, ttl))
}
...
func openAIUsageFromGJSON(value gjson.Result) (OpenAIUsage, bool) {
	...
	audioOutputTokens := firstPositiveGJSONInt(
