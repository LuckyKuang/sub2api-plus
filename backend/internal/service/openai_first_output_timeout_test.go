...
func TestOpenAINativeFirstOutputEOFDispatchesTerminalEventWithoutBlankLine(t *testing.T) {
	...
	require.NoError(t, err)
	require.NotNil(t, result)
	require.Nil(t, result.firstTokenMs, "terminal lifecycle without content is not a token")
	require.Nil(t, result.firstOutputMs, "terminal lifecycle without content is not output")
