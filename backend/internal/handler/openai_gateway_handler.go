...
func openAIForwardSucceededForScheduling(forwardErr error, result *service.OpenAIForwardResult) bool {
	return forwardErr == nil && result.SucceededForScheduling()
}
...
func (h *OpenAIGatewayHandler) Responses(c *gin.Context) {
	...
	modelResult := gjson.GetBytes(securityAuditBody, "model")
	...
	rawStream, ok := parseOpenAICompatibleStream(securityAuditBody)
	if !ok {
	...
	setOpsRequestContext(c, reqModel, rawStream)
	setOpsEndpointContext(c, "", int16(service.RequestTypeFromLegacy(rawStream, false)))

	if decision := h.checkSecurityAudit(c, reqLog, apiKey, subject, service.ContentModerationProtocolOpenAIResponses, reqModel, securityAuditBody); decision != nil && !decision.AllowNextStage {
	...
	// body-signal compact：审核通过后再启动 SSE 注释行心跳，防止阻断式
	// 审核等待期间提前提交 200 响应并破坏 503 fail-closed 错误语义。
	...
	for {
		...
		if err != nil {
			...
			if result != nil && result.ImageCount > 0 {
				...
				// Cyber policy paths already own their dedicated usage record. A
				// non-cyber partial result falls through to the single normal
				// RecordUsage submission below with IsComplete=false.
...
func (h *OpenAIGatewayHandler) ResponsesWebSocket(c *gin.Context) {
	...
	if ingressLeaseErr != nil {
		...
		closeOpenAIClientWS(wsConn, coderws.StatusTryAgainLater, "WebSocket ingress capacity is temporarily unavailable")
		return
	}
		...
		hooks := &service.OpenAIWSIngressHooks{
			...
			BeforeTurn: func(turn int) error {
				...
				if snapshot := turnChannelMapping.Load(); snapshot != nil && snapshot.turn == turn {
					if model := strings.TrimSpace(snapshot.mapping.MappedModel); model != "" {
						turnModel = model
					}
				}
			...
			AfterTurn: func(turn int, result *service.OpenAIForwardResult, turnErr error) {
				...
				// 排除 spark 影子:其 codex_* 仅由 QueryUsage(/wham/usage bengalfox)更新(外审第7轮 P1)。
				if account.Type == service.AccountTypeOAuth && !account.IsShadow() {
					h.gatewayService.ApplyCodexUsageSnapshotFromResult(ctx, account.ID, result)
				}
...
func (h *OpenAIGatewayHandler) handleFailoverExhausted(c *gin.Context, failoverErr *service.UpstreamFailoverError, streamStarted bool) {
	...
	if failoverErr.IsOpenAIProxyRetryBufferLimit() {
		service.SetOpsUpstreamError(c, http.StatusRequestEntityTooLarge, service.OpenAIProxyRetryBufferLimitClientMessage, "")
		h.handleStreamingAwareError(
			c,
			http.StatusRequestEntityTooLarge,
			"invalid_request_error",
