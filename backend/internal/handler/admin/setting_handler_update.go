...
type UpdateSettingsRequest struct {
	...
	AsyncImageUserImagesPerMinute             int                               `json:"async_image_user_images_per_minute"`
...
func (h *SettingHandler) UpdateSettings(c *gin.Context) {
	...
	if req.OpenAICodexClientVersion != nil {
		...
		if normalized != "" && service.CompareVersions(normalized, service.OpenAICodexUpstreamMinVersion) < 0 {
			response.Error(c, http.StatusBadRequest, "openai_codex_client_version must be at least "+service.OpenAICodexUpstreamMinVersion)
			return
		}
	...
	settings := &service.SystemSettings{
		...
		AsyncImageUserImagesPerMinute:          req.AsyncImageUserImagesPerMinute,
		...
		OpenAICodexLocalGroupQuotaEnabled: func() bool {
			...
			return previousSettings.OpenAICodexLocalGroupQuotaEnabled
		}(),
	...
	payload := dto.SystemSettings{
		...
		AsyncImageUserImagesPerMinute:                          updatedSettings.AsyncImageUserImagesPerMinute,
