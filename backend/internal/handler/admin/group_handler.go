...
import (
	...
	"github.com/LuckyKuang/sub2api-plus/internal/platform/liveattestation"
...
func sanitizeCreateGroupRequestForSimpleMode(req *CreateGroupRequest) {
	...
	allowed.LongContextPricingEnabled = new(false)
...
func (h *GroupHandler) GetGroupAPIKeys(c *gin.Context) {
	...
	outKeys := make([]dto.AdminAPIKeySummary, 0, len(keys))
	for i := range keys {
		outKeys = append(outKeys, *dto.AdminAPIKeySummaryFromService(&keys[i]))
	}
