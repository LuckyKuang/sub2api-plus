...
func (h *AccountHandler) ApplyOAuthCredentials(c *gin.Context) {
	...
	// Re-authorization only returns newly issued OAuth material. The normal
	// account editor owns user_agent, and intentionally does not include it in
	// this payload, so preserve a configured account-level Codex identity.
	if existing.Platform == service.PlatformOpenAI && !existing.IsCredentialShadow() {
		if priorUserAgent := existing.GetOpenAIUserAgent(); priorUserAgent != "" {
			if _, supplied := req.Credentials["user_agent"]; !supplied {
				req.Credentials["user_agent"] = priorUserAgent
			}
		}
	}
