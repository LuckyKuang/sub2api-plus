...
import (
	...
	dbaccount "github.com/LuckyKuang/sub2api-plus/ent/account"
	dbapikey "github.com/LuckyKuang/sub2api-plus/ent/apikey"
	dbgroup "github.com/LuckyKuang/sub2api-plus/ent/group"
	"github.com/LuckyKuang/sub2api-plus/ent/schema/mixins"
	dbuser "github.com/LuckyKuang/sub2api-plus/ent/user"
	dbusersub "github.com/LuckyKuang/sub2api-plus/ent/usersubscription"
...
const usageLogSelectColumns = "id, user_id, api_key_id, account_id, request_id, model, requested_model, upstream_model, upstream_response_model, upstream_model_mismatch, group_id, subscription_id, input_tokens, output_tokens, cache_creation_tokens, cache_read_tokens, cache_creation_5m_tokens, cache_creation_1h_tokens, image_output_tokens, audio_output_tokens, image_output_cost, image_input_tokens, image_input_cost, input_cost, output_cost, cache_creation_cost, cache_read_cost, total_cost, actual_cost, rate_multiplier, account_rate_multiplier, billing_type, request_type, stream, openai_ws_mode, duration_ms, first_token_ms, last_token_ms, first_output_ms, first_output_kind, is_complete, user_agent, ip_address, image_count, image_size, image_input_size, image_output_size, image_size_source, image_size_breakdown, video_count, video_resolution, video_duration_seconds, service_tier, reasoning_effort, requested_reasoning_effort, inbound_endpoint, upstream_endpoint, cache_ttl_overridden, long_context_billing_applied, channel_id, model_mapping_chain, billing_tier, billing_mode, account_stats_cost, upstream_request_id, session_id, completion_status, usage_source, native_compaction_v2, timing_version, created_at, codex_rollout_budget_units"
...
func (r *usageLogRepository) ListWithFilters(ctx context.Context, params pagination.PaginationParams, filters UsageLogFilters) ([]service.UsageLog, *pagination.PaginationResult, error) {
	...
	if value := strings.TrimSpace(filters.CompletionStatus); value != "" {
		conditions = append(conditions, fmt.Sprintf("completion_status = $%d", len(args)+1))
		args = append(args, value)
	}
	if value := strings.TrimSpace(filters.UsageSource); value != "" {
		conditions = append(conditions, fmt.Sprintf("usage_source = $%d", len(args)+1))
		args = append(args, value)
	}
...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	var (
		...
		timingVersion             int
		createdAt                 time.Time
		codexRolloutBudgetUnits   sql.NullFloat64
	)
	...
	log := &service.UsageLog{
		...
		CreatedAt:                 createdAt,
		CodexRolloutBudgetUnits:   nullFloat64Ptr(codexRolloutBudgetUnits),
	}
	...
	if upstreamModelMismatch.Valid {
		log.UpstreamModelMismatch = &upstreamModelMismatch.Bool
	}
	if channelID.Valid {
