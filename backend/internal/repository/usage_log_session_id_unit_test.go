...
// precedes completion_status, usage_source, and created_at.
func TestPrepareUsageLogInsert_SessionIDArgWiring(t *testing.T) {
	require.Len(t, usageLogInsertArgTypes, 66, "arg-type table must include usage completion metadata")
	...
	require.Equal(t, service.UsageCompletionUnknown, prepared.args[len(prepared.args)-3])
	require.Equal(t, service.UsageSourceUnknown, prepared.args[len(prepared.args)-2])
}
