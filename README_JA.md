...
> Sub2API Plus は元の Sub2API プロジェクトを基にした独立したコミュニティ管理のフォークです。
> 上流の公式リリースではなく、
...
今後の Git タグと GitHub Release は `vX.Y.Z+custom.NNN` を使用します。
OCI イメージタグでは先頭の `v` を維持し、Docker Tag で使用できない
`+` のみを `-` に変換します。
...
マージした場合は `001` に戻します。本番環境では固定 GHCR
バージョンタグを使用し、`latest` は常に最新のカスタムリリースへ追従する
場合にのみ使用してください。詳細は [UPSTREAM.md](UPSTREAM.md) を参照してください。
