...
> Sub2API Plus は [Wei-Shaw/sub2api](https://github.com/Wei-Shaw/sub2api)
> を基にした独立したコミュニティ管理のフォークです。上流の公式リリースではなく、
> 上流プロジェクトとの提携、承認、支援、商標利用許諾を意味するものではありません。
...
- **📖 ライセンスと責任**：本プロジェクトは LGPL-3.0-or-later で提供されます。デプロイおよび運用の主体は、適用法、上流サービスの規約、データ保護義務、運用上のリスクを自ら評価して負担してください。プロジェクト名および上流名は、商用許可、商標利用許諾、提携または推奨を意味しません。

## 概要

Sub2API Plus は、AI 製品のサブスクリプションから API クォータを配分・管理するために設計された AI API ゲートウェイプラットフォームです。ユーザーはプラットフォームが生成した API キーを通じて上流の AI サービスにアクセスでき、プラットフォームは認証、課金、負荷分散、リクエスト転送を処理します。
...
Sub2API Plus を拡張・統合するコミュニティプロジェクト:

| プロジェクト | 説明 | 機能 |
|---------|-------------|----------|
| ~~[Sub2ApiPay](https://github.com/touwaeriol/sub2apipay)~~ | ~~セルフサービス決済システム~~ | **内蔵済み** — 決済機能は Sub2API Plus に統合されました。別途デプロイは不要です。[決済設定ガイド](docs/PAYMENT.md)をご参照ください |
...
Sub2API Plus（または CRS）を Nginx でリバースプロキシし、Codex CLI と組み合わせて使用する場合、Nginx の `http` ブロックに以下の設定を追加してください:
...
Apple シリコン搭載 Mac と macOS 26 では、Apple `container` 1.1.0 以降を使用して Sub2API Plus、PostgreSQL、Redis の完全なスタックを実行できます:
...
- `security.trust_forwarded_ip_for_api_key_acl` - 従来の生転送ヘッダーによる上書きを制御（アップグレード互換性のため既定で有効）。無効にすると `server.trusted_proxies` を厳格に使用し、Sub2API Plus に直接接続するプロキシの正確な CIDR のみを指定
...
Sub2API Plus は [Antigravity](https://antigravity.so/) アカウントをサポートしています。認証後、Claude および Gemini モデル用の専用エンドポイントが利用可能になります。
...
元の上流プロジェクトの著作権およびライセンス表示を保持しています。
