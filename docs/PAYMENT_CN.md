...
Sub2API Plus 内置支付系统，支持用户自助充值，无需部署独立的支付服务。
...
> **易支付服务商**：可配置任何兼容易支付协议、并适合自身结算地区与
> 币种的服务商。请独立评估服务商的安全性、稳定性、费率、服务条款及合规性；
> Sub2API Plus 不对任何第三方支付服务商做担保或背书。
...
| 对比项 | Sub2ApiPay | 内置支付 |
|--------|-----------|---------|
| 部署方式 | 独立服务（Next.js + PostgreSQL） | 内置于 Sub2API Plus，无需额外部署 |
| 支付方式 | EasyPay、支付宝、微信、Stripe | 相同 |
| 配置方式 | 环境变量 + 独立管理后台 | Sub2API Plus 管理后台内统一配置 |
| 充值对接 | 通过 Admin API 回调 | 内部直接处理，更可靠 |
| 订阅套餐 | 支持 | 暂不支持（计划中） |
| 订单管理 | 独立管理界面 | 集成在 Sub2API Plus 管理后台 |

### 迁移步骤

1. 在 Sub2API Plus 管理后台启用支付并配置服务商（使用相同的支付凭证）
2. 更新 Webhook 回调地址为 Sub2API Plus 的回调地址
3. 确认新订单通过内置支付正常处理
4. 停用 Sub2ApiPay 服务
