...
1. **H2** — Migration 203 (official 195; Plus renumbered) and code defaults set `channel_monitor_mode=v2`, so every upgrade without an existing key auto-opts into passive V2, stops V1 probes, and may leave users on an empty V2 view.
