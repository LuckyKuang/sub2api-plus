...
The application version for this custom branch is `v0.1.165+custom.004`.
Use `ghcr.io/luckykuang/sub2api-plus:v0.1.165-custom.004` when building or
publishing an OCI image:

```bash
docker build \
  --build-arg VERSION=0.1.165+custom.004 \
  --tag ghcr.io/luckykuang/sub2api-plus:v0.1.165-custom.004 \
  .
```

After that image is available to the Apple `container` runtime, set
`APPLE_CONTAINER_SUB2API_IMAGE=ghcr.io/luckykuang/sub2api-plus:v0.1.165-custom.004`. Until then, keep
