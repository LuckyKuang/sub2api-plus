...
Apple/OCI image:    ghcr.io/luckykuang/sub2api-plus:v0.2.1-custom.001
```
...
docker build \
  ...
  --tag ghcr.io/luckykuang/sub2api-plus:v0.2.1-custom.001 \
  .
```

After that image is available to the Apple `container` runtime, set
`APPLE_CONTAINER_SUB2API_IMAGE=ghcr.io/luckykuang/sub2api-plus:v0.2.1-custom.001`. Until then, keep
