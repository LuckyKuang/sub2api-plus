...
The application version for this custom branch is `v0.1.165+custom.002`.
Use Docker's tag-safe equivalent, `ghcr.io/luckykuang/sub2api-plus:0.1.165-custom.002`, when building
or publishing an OCI image:

```bash
docker build \
  --build-arg VERSION=0.1.165+custom.002 \
  --tag ghcr.io/luckykuang/sub2api-plus:0.1.165-custom.002 \
  .
```

After that image is available to the Apple `container` runtime, set
`APPLE_CONTAINER_SUB2API_IMAGE=ghcr.io/luckykuang/sub2api-plus:0.1.165-custom.002`. Until then, keep
