...
```text
Git/GitHub: v0.1.182+custom.001
GHCR:       ghcr.io/luckykuang/sub2api-plus:v0.1.182-custom.001
```
...
- Upgrades from the latest published Plus release, `v0.1.178+custom.005`, to
  `v0.1.182+custom.001` additionally apply migrations `229` through `233`:
...
```bash
curl -sSL https://raw.githubusercontent.com/LuckyKuang/sub2api-plus/main/deploy/install.sh | sudo bash -s -- install --version 'v0.1.182+custom.001'
```
...
```bash
sudo ./install.sh install --version 'v0.1.182+custom.001'
```
