    ...
    >
      ...
      >
        <div class="flex items-center justify-between gap-4">
          <div>
            <label class="input-label mb-0">{{ t('admin.accounts.openai.oauthSessionSharing') }}</label>
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
      ...
      >
        <div class="flex items-center justify-between gap-4">
          ...
          <Toggle
            :model-value="!!openAILongContextBillingEnabled" @update:model-value="toggleOpenAILongContextBilling"
...
const withAntigravityConfirmFlag = (payload: CreateAccountRequest): CreateAccountRequest => {
  const normalizedPayload = {
    ...payload,
    extra: withUpstreamRequestIdHeader(payload.extra)
  }
  ...
  const cloned = { ...normalizedPayload }
...
const buildOpenAIExtra = (base?: Record<string, unknown>): Record<string, unknown> | undefined => {
  ...
  if (form.type === 'oauth' && openaiOAuthSessionSharingEnabled.value) {
    extra.openai_oauth_session_policy = {
      enabled: true,
      allowed_group_ids: [...form.group_ids]
    }
  } else {
...
const withUpstreamRequestIdHeader = (
