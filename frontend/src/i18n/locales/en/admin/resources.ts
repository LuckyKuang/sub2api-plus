export default {
    ...
    usageAlert: {
      ...
      menu: 'Usage Alert',
      description:
        'Push this account\'s usage windows (5h / 7d) to WeCom, Feishu, or a custom webhook on a cron schedule. Multiple rules can run together; with a threshold, alerts fire only when utilization reaches it. Runs on the server without an admin browser session.',
      rulesTitle: 'Alert rules',
      addRule: 'Add rule',
      emptyRules: 'No rules yet. Click “Add rule” to start.',
      ruleLabel: 'Rule {n}',
      enabled: 'Enable this rule',
      enabledHint: 'When enabled, the backend runs this rule on its cron schedule',
      channel: 'Bot type',
      ...
      cronTooltipMeaning: 'Defines when the rule runs automatically.',
      ...
      thresholdEnabledHint: 'Off: push on every cron tick. On: alert only when max utilization ≥ threshold.',
