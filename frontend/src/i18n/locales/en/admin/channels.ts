export default {
    ...
    riskControl: {
      ...
      textApiMode: 'Text Moderation API policy',
      textApiModes: {
        auto: 'Automatic',
        autoObserve: 'Automatic observe',
        blocking: 'Blocking',
        normalObserve: 'Standard observe',
        observe: 'Shadow observe',
        off: 'Text API off',
      },
      textApiModeDesc: {
        auto: 'When blocking Prompt Guard covers this request, text moderation becomes shadow-only; otherwise it remains blocking. Images always follow the global mode.',
        blocking: 'The Moderation API judges text under the global mode. Images remain independently moderated.',
        observe: 'Text API findings are recorded for comparison only: no blocking, risk hash, notification, violation count, or ban. Images still follow the global mode.',
        off: 'Text does not call the Moderation API. Images still follow the global mode; local keyword and hash rules remain active.',
      },
      policyHelp: {
        open: 'View the complete global mode and text API policy matrix',
        title: 'Content moderation policy combinations',
        summary: 'The global mode determines whether Content Moderation runs synchronously or asynchronously and its maximum blocking authority. The text API policy only refines text Moderation API calls and decisions. Images, keywords, and hashes follow their own controls.',
        current: 'Effective behavior: ',
        globalMode: 'Global mode',
        textPolicy: 'Text API policy',
        behavior: 'Effective behavior',
        canBlock: 'Can block this request',
        modeLabels: { pre_block: 'Pre-block', observe: 'Observe only', off: 'Off' },
        policyLabels: { auto: 'Automatic', blocking: 'Blocking', observe: 'Shadow observe', off: 'Text API off', any: 'Any policy' },
        blockLabels: { depends: 'Depends on the request', yes: 'Yes', no: 'No', textNoImageYes: 'Text no, images can', textNo: 'Text API no' },
        combinations: {
          pre_block: {
            auto: 'Uses shadow observation when Prompt Guard can block this request; otherwise calls Moderation synchronously.',
            blocking: 'Calls text Moderation synchronously and blocks policy hits.',
            observe: 'Records text asynchronously in shadow mode; images remain synchronously moderated.',
            off: 'Skips text Moderation; images, keywords, and hashes still follow their own controls.',
          },
          observe: {
            auto: 'Uses standard or shadow asynchronous review according to Prompt Guard authority and never blocks this request.',
            blocking: 'The global mode limits this to standard asynchronous review; it does not block synchronously.',
            observe: 'Records an asynchronous shadow result without notifications, hashes, violation counts, or bans.',
            off: 'Skips text Moderation; images remain asynchronously moderated by the global mode.',
          },
          off: { any: 'Skips the entire Content Moderation path; the text policy is temporarily inactive.' },
        },
      },
      endpointPool: {
        title: 'Moderation endpoint pool',
        hint: 'Endpoints are called in list order. A platform failure starts cooldown and immediately fails over. Cooldown expiry sends no probe; the next real moderation request verifies recovery.',
        add: 'Add endpoint',
        unnamed: 'Unnamed endpoint',
        defaultName: 'Moderation endpoint {index}',
        name: 'Endpoint name',
        apiKeys: 'API Keys',
        keysKeep: '{count} keys are stored; leave empty to keep them',
        keysRequired: 'Enter at least one API Key',
        keysHint: 'Saving non-empty input replaces this endpoint\'s keys. Full secrets are never returned to the page.',
        cooldown: 'Cooldown (seconds)',
        failureThreshold: 'Consecutive failure threshold',
        test: 'Test',
        testSuccess: '{name} test succeeded',
        testFailed: 'Endpoint test failed',
        pause: 'Pause',
        resume: 'Resume',
        paused: 'Endpoint paused',
        resumed: 'Endpoint resumed; the next real request will verify availability',
        pauseFailed: 'Failed to update endpoint pause state',
        moveUp: 'Increase priority',
        moveDown: 'Decrease priority',
        delete: 'Delete endpoint',
        status: { healthy: 'Healthy', degraded: 'Degraded', error: 'Error', cooldown: 'Automatic cooldown', manual_pause: 'Manually paused', half_open: 'Recovery check', disabled: 'Disabled' },
      },
      ...
      auditTestImagesHint: 'Upload, drag, or paste images. Up to 16 images, 8MB each.',
      ...
      sessionBlock: 'Block session after API hit',
      sessionBlockHint: 'Only an external Moderation API text/image threshold hit seeds the session blacklist. Keyword hits do not. Admins are blocked for the current request only. A missing session ID never bans the account. Off by default.',
      sessionBlockTTL: 'Session block TTL (seconds)',
      sessionBlockTTLHint: 'Default 2592000 (30 days). Hits do not refresh TTL. Range 60–7776000.',
      sessionBlocks: 'Blocked sessions',
      sessionBlocksHint: 'Unblock by original session ID. Unbanning a user does not clear session blocks.',
      sessionBlockCount: 'Blocked sessions: {count}',
      sessionBlockSearch: 'Search session ID / user / key',
      deleteSessionBlock: 'Unblock session',
      clearSessionBlocks: 'Clear all session blocks',
      clearSessionBlocksConfirm: 'Clear all content-moderation session blocks? Audit records are kept.',
      sessionBlockDeleted: 'Unblocked {count} session(s)',
      sessionBlockNotFound: 'Session block not found',
      sessionBlockDeleteFailed: 'Failed to unblock session',
      sessionBlocksCleared: 'Cleared {count} session blocks',
      sessionBlocksClearFailed: 'Failed to clear session blocks',
      sessionBlockEmpty: 'No blocked sessions',
      sessionBlockExpires: 'Expires',
      cyberPolicyAutoBan: 'Auto-ban on cyber_policy',
      cyberPolicyAutoBanHint: 'When enabled, an upstream cyber_policy hit immediately stops access. Regular users are disabled; admins only have the triggering API key disabled. Off by default.',
      ...
      extractionStatus: 'Content Extraction Coverage',
      extractionStatusHint: 'Every applicable request must be classified as extracted, explicitly text-free, or failed. Any failed count needs investigation.',
      extractionAttempted: 'Attempted',
      extractionSucceeded: 'Succeeded',
      extractionEmpty: 'No Text',
      ...
      keywordModeKeywordOnlyDesc: 'Use keywords only for text; misses skip the text API. Images still follow the global moderation mode.',
      keywordModeKeywordOnlyNotice: 'Keyword-only text strategy: unmatched text-only requests pass without the API; images are still moderated.',
    ...
    subscriptions: {
      ...
      resetQuotaConfirm: "Reset the 5-hour, daily, weekly, and monthly usage quota for '{user}'? Usage will be zeroed and windows restarted now.",
