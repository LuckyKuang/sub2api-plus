export default {
    ...
    backup: {
      ...
      s3: {
        ...
        secretGuideLink: 'View setup steps',
      ...
      encryptionKeyGuide: {
        title: 'Configure a fixed encryption key',
        intro: 'Before saving a new S3 Secret Access Key (backup or async image storage), the process must already have a fixed encryption key. If none is set, a temporary key is generated on every start and stored ciphertext cannot be decrypted after a restart, so the save is rejected.',
        whenNeeded: 'You do not need this change if you only click Test Connection, or if you leave Secret empty to keep the stored value.',
        generateTitle: 'Generate the key once',
        generateHint: 'The output should be 64 hex characters. Copy it into the config file. Do not put this command in a script that runs on every start.',
        ...
        dockerWhere: 'Set TOTP_ENCRYPTION_KEY in deploy/.env.',
        dockerStep1: 'Go to the deploy directory (usually deploy/ in the repository).',
        dockerStep2: 'Check for an existing key: grep \'^TOTP_ENCRYPTION_KEY=\' .env',
        dockerStep3: 'Installs created by docker-deploy.sh usually already have a value. If it is already a 64-character hex string, do not overwrite it — recreate the container as below and save the Secret.',
        dockerStep4: 'If it is missing, run: TOTP_ENCRYPTION_KEY=$(openssl rand -hex 32) then echo "TOTP_ENCRYPTION_KEY=$TOTP_ENCRYPTION_KEY" >> .env',
        dockerStep5: 'If the line exists but is empty, edit that line. Do not append a second line.',
        ...
        binaryWhere: 'Set totp.encryption_key in /opt/sub2api/config.yaml. That is the file written by install.sh and the setup wizard.',
        binaryStep1: 'Confirm the service exists: systemctl status sub2api',
        binaryStep2: 'Look for an existing key: sudo grep -n encryption_key /opt/sub2api/config.yaml',
        binaryStep3: 'If it is already a 64-character hex string, go back and save the Secret. Do not generate another key.',
        binaryStep4: 'If it is missing, generate one: openssl rand -hex 32 and copy the output.',
        binaryStep5: 'Edit the config: sudo nano /opt/sub2api/config.yaml',
        binaryYamlHint: 'If there is no totp: section, append the block at the end. If totp: already exists, only add or correct encryption_key.',
        binaryYaml: 'totp:\n  encryption_key: "paste-the-64-hex-chars-here"',
        binaryStep6: 'sudo chown sub2api:sub2api /opt/sub2api/config.yaml && sudo chmod 600 /opt/sub2api/config.yaml',
        binaryRestart: 'sudo systemctl restart sub2api',
        binaryStatus: 'sudo systemctl status sub2api',
        binaryLogs: 'sudo journalctl -u sub2api -n 50 --no-pager',
        binaryLogHint: 'The logs should no longer contain TOTP encryption key auto-generated.',
        ...
        standaloneStep1: 'Write the same totp.encryption_key field used by the official binary.',
        standaloneStep2: 'Stop the old process and start a new one. Reloading the web page does not load this key.',
        afterTitle: 'Save the Secret after restart',
        afterHint: 'Return to this page, enter the Secret Access Key again, and save. Saving settings in the web UI does not load this key.',
        warningsTitle: 'Do not do this',
        warningExisting: 'If you already have TOTP users or a stored S3 Secret, do not replace the key currently in use or the old data cannot be decrypted.',
        warningOnce: 'Run openssl rand only once. Do not generate a new key on every start.',
        warningNotMinio: 'This key is not the MinIO/S3 Access Key. Object-storage credentials may change; the encryption key must stay fixed.',
        warningWrongFile: 'Docker uses deploy/.env. The official binary uses /opt/sub2api/config.yaml. Do not edit a different config file.'
      }
    },
    ...
    groups: {
      ...
      subscription: {
        ...
        quotaFollowReset: {
          ...
          hint: 'Select a bound OpenAI OAuth account. Enabling, re-enabling or changing the source first establishes a fresh official weekly baseline without resetting quotas. Each subsequent confirmed natural or early official reset clears usage once. Sources are refreshed in the background; observing zero usage is not required. Historical spending is not replayed or backfilled. Account routing is unchanged.',
          ...
          waitingBaseline: 'Fetching a fresh official weekly baseline for this activation. Old baselines will not be reused and quotas will not reset immediately.',
