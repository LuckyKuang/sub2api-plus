export default {
  promptAudit: {
    ...
    pool: {
      ...
      timeoutRange: 'Supported range: {min}–{max} ms.', inputLimitRange: 'Supported range: {min}–{max} Unicode characters.', inputLimitBehavior: 'The current user text is split when it exceeds this value. To submit one message intact, set the limit no lower than its character count and ensure the model context is sufficient. When several nodes are enabled, the smallest limit applies.',
    ...
    events: {
      title: 'Audit events', description: 'Review events by identity, IP, route, risk, hash, and time; the detail view shows the retained audit content.', decision: 'Decision', risk: 'Risk level', endpoint: 'Endpoint', clientIp: 'Client IP', filterByIp: 'Filter by IP {ip}', groupId: 'Group ID', userId: 'User ID', apiKeyId: 'API Key ID', keyword: 'Keyword',
      ...
      promptFull: 'Retained audit content (unredacted)',
      promptFullHint: 'The retained audit content is stored with this event for admin review only and may be truncated at the storage limit. Treat it as sensitive data and do not share it.',
      promptTruncatedWarning: 'This event was truncated at the storage limit or originated before complete diagnostics were available. The omitted portion cannot be recovered.',
      ...
      score: 'Score',
      categories: 'Categories', model: 'Model', stage: 'Request stage', executionMode: 'Execution mode', promptSize: 'Audited size', promptSizeValue: '{chars} characters · {messages} messages', contentStatus: 'Retention status', contentComplete: 'Retained within limit', contentTruncated: 'Truncated', noRisks: 'No derived risk summaries for this event.',
      requestId: 'Request ID', promptHash: 'Prompt SHA-256',
      technical: {
        scanner: 'Scanner', policy: 'Policy', guardEndpoint: 'Guard endpoint', config: 'Config',
        chunks: 'Chunks', matchedChunk: 'Matched chunk', inputLimit: 'Characters per chunk', latency: 'Audit latency', protocol: 'Protocol',
      },
