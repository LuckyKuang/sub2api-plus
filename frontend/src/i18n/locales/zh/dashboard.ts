export default {
  ...
  keys: {
    ...
    useKeyModal: {
      ...
      grok: {
        description:
          '配置 Grok CLI、Claude Code、Codex 或 OpenCode，让请求通过当前 Sub2API Plus Grok 分组发送。文本模型走 Responses；图片/视频使用 Imagine 模型 ID 与媒体端点。',
        claudeDescription: '配置 Claude Code，让 Messages API 请求通过当前 Sub2API Plus Grok 分组发送。',
        codexDescription: '配置 Codex，让 Responses API 请求通过当前 Sub2API Plus Grok 分组发送。',
        configTomlHint:
  ...
  usage: {
    ...
    latencyFirstImage: '首图数据',
    latencyFirstAudio: '首音频数据',
    ...
    timingUnavailableNonStream: '非流式请求，TPS 使用解码窗（末 Token − 首字）',
    ...
    timingUnavailableShort: '低置信样本：解码窗不足 300ms 或输出不足 8 tokens',
    timingUnavailableReason: '不可计算原因',
    latencyTpsHint: '解码速率：计费文本 Token ÷（末 Token 时刻 − 首字）。不含思考等待（已由首字单独展示），不含末 Token 之后的收尾。未完成、非流式、短解码窗仍显示数字，并标明置信说明。',
	incomplete: '未完成',
	incompleteHint: '请求在完整终态前结束，当前显示的用量和费用可能只是部分结果。',
	clientDisconnected: '客户端已断开',
	clientDisconnectedHint: '上游接受请求后客户端断开，当前用量和费用来自结算阶段取得的实际或部分 usage。',
    time: '时间',
    ws: 'WS',
    stream: '流式',
    sync: '同步',
    ...
    tabs: { usage: '用量明细', errors: '错误请求', ranking: '用户排行', disconnects: '客户端中断' },
    errors: {
