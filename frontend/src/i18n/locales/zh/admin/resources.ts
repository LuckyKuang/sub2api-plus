export default {
    ...
    usageAlert: {
      ...
      menu: '用量告警',
      description:
        '按 cron 定时推送用量窗口；可另开阈值监听（独立 cron + 冷却窗口）。支持免打扰时段。多条规则可同时生效，任务在服务端执行。',
      rulesTitle: '告警规则',
      addRule: '添加规则',
      emptyRules: '暂无规则，点击「添加规则」开始配置。',
      ruleLabel: '规则 {n}',
      enabled: '启用本规则',
      enabledHint: '开启后，后台会按本规则的定时 cron 推送用量报告',
      channel: '机器人类型',
      channelWecom: '企业微信机器人',
      channelFeishu: '飞书机器人',
      channelCustom: '自定义机器人',
      ...
      webhookHintWecom: '企业微信群机器人 Webhook，完整展示与保存，不做脱敏。',
      webhookHintFeishu: '飞书 / Lark 自定义机器人 Webhook，完整展示与保存。',
      webhookHintCustom: '任意 HTTPS URL；将收到 JSON（title / markdown / account 字段等）。',
      cronExpression: '定时推送 Cron',
      cronPlaceholder: '0 * * * *',
      cronHelp: '标准 5 字段 cron（分钟 小时 日 月 星期），用于定时推送用量窗口报告',
      cronTooltipTitle: 'Cron 示例：',
      cronTooltipMeaning: '定义定时推送时间规则。',
      cronTooltipExampleHourly: '0 * * * *：每小时整点',
      cronTooltipExampleDaily: '0 9 * * *：每天 09:00',
      thresholdEnabled: '启用用量阈值',
      thresholdEnabledHint: '开启后按「阈值监听 Cron」检查使用率；达到阈值且不在冷却窗口内才告警一次。',
      thresholdPercent: '阈值（%）',
      thresholdPercentHint: '填写 1–99 的整数。达到阈值时消息标题为「用量阈值告警」。',
      thresholdWatchCron: '阈值监听 Cron',
      thresholdWatchCronPlaceholder: '*/5 * * * *',
      thresholdWatchCronHint: '必填。后台按此 cron 拉取用量并判断是否超过阈值。',
      cooldownSeconds: '冷却窗口（秒）',
      cooldownSecondsHint: '必填。告警成功后，冷却期内不再重复告警，也不再频繁拉取用量；冷却结束后恢复监听。',
      quietHours: '免打扰时间',
      quietHoursHint: '选填。每天这些时段内，定时推送与阈值监听都不会拉取用量。支持跨午夜（如 22:00:00-06:00:00）。',
      quietHoursAdd: '添加时段',
      quietHoursStart: '开始',
      quietHoursEnd: '结束',
      quietHoursEmpty: '未设置免打扰（全天可推送/监听）',
      forceProbe: '强制探测上游',
      forceProbeHint: '开启后每次执行前主动探测上游配额（可能消耗探测配额）；关闭则优先使用本地快照。',
      nextRun: '下次定时推送',
      lastRun: '上次定时推送',
      thresholdNextRun: '下次阈值监听',
      lastThresholdAlert: '上次阈值告警',
      lastError: '上次错误',
      testSend: '测试推送',
      testing: '推送中...',
      saveSuccess: '用量告警配置已保存',
      testSuccess: '已推送到对应机器人'
    },
    ...
    usage: {
	  ...
	  usageSource: '用量来源',
	  allUsageSources: '全部用量来源',
	  usageSourceExact: '上游精确值',
	  usageSourcePartial: '部分用量',
	  usageSourceEstimated: '估算用量',
	  usageSourceReconciled: '对账补录',
      ipAddress: 'IP',
      clickToViewBalance: '点击查看充值记录',
      failedToLoadUser: '加载用户信息失败',
      userDeletedBadge: '已删除',
      disconnectEvents: {
        hint: '这里只展示生命周期元数据；连续结果按清洗后的客户端入站会话独立计算。',
        userAccount: '用户账号',
        ...
        usageMissing: '缺少用量',
        usageSource: 'Usage 来源',
        allSources: '全部来源',
        upstreamExact: '上游精确用量',
        partial: '部分用量',
        estimated: '估算用量',
        reconciled: '已对账',
        autoBanned: '自动封禁',
        consecutiveCount: '连续取消次数',
        threshold: '阈值',
        enforcement: '执行模式',
        action: '处置',
        auditOnly: '仅审计',
