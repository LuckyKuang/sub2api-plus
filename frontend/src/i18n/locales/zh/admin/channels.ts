export default {
    ...
    riskControl: {
      ...
      textApiMode: '文本 Moderation API 策略',
      textApiModes: {
        auto: '自动',
        autoObserve: '自动观察',
        blocking: '同步裁决',
      ...
      textApiModeDesc: {
        auto: '仅当同步 Prompt Guard 确实覆盖当前请求时，文本 API 改为影子记录；否则仍同步裁决。图片始终按全局模式审核。',
        blocking: '文本由 Moderation API 按全局模式裁决；图片保持独立审核。',
        observe: '文本 API 只记录对比结果，不拦截、不写风险 Hash、不通知、不累计违规或封禁；图片仍按全局模式裁决。',
        off: '文本不调用 Moderation API；图片仍按全局模式裁决，本地关键词和 Hash 规则不受影响。',
      },
      policyHelp: {
        open: '查看全局模式与文本 API 策略的完整组合说明',
        title: '内容审核策略组合说明',
        summary: '全局模式决定 Content Moderation 是否运行、同步或异步，以及最高阻断权限；文本 API 策略只细化文本 Moderation API 的调用和裁决方式。图片、关键词与 Hash 仍按各自配置执行。',
        current: '实际行为：',
        globalMode: '全局模式',
        textPolicy: '文本 API 策略',
        behavior: '实际行为',
        canBlock: '能否阻断当前请求',
        modeLabels: { pre_block: '前置拦截', observe: '仅观察', off: '关闭' },
        policyLabels: { auto: '自动', blocking: '同步裁决', observe: '影子观察', off: '关闭文本 API', any: '任意策略' },
        blockLabels: { depends: '视请求而定', yes: '是', no: '否', textNoImageYes: '文本否、图片可以', textNo: '文本 API 否' },
        combinations: {
          pre_block: {
            auto: 'Prompt Guard 对当前请求有阻断权限时执行影子观察，否则同步调用 Moderation。',
            blocking: '同步调用文本 Moderation，命中策略时阻断请求。',
            observe: '文本异步影子记录；图片仍按前置模式同步审核。',
            off: '不调用文本 Moderation；图片、关键词和 Hash 仍按各自配置执行。',
          },
          observe: {
            auto: '根据 Prompt Guard 权限进入普通异步审核或影子异步审核，不阻断当前请求。',
            blocking: '受全局模式限制，实际执行普通异步审核，不同步阻断。',
            observe: '异步影子记录，不通知、不写入 Hash、不累计违规、不封禁。',
            off: '不调用文本 Moderation；图片按全局模式异步审核。',
          },
          off: { any: '整个 Content Moderation 流程跳过，文本策略暂时不生效。' },
        },
      },
      endpointPool: {
        title: 'Moderation 审计池',
        hint: '按列表优先级调用。平台异常后自动冷却并立即切换到下一平台；冷却结束不会主动探测，下一笔真实审核请求负责恢复验证。',
        add: '添加平台',
        unnamed: '未命名平台',
        defaultName: '审核平台 {index}',
        name: '平台名称',
        apiKeys: 'API Keys',
        keysKeep: '已保存 {count} 个 Key；留空保持不变',
        keysRequired: '请输入至少一个 API Key',
        keysHint: '输入新 Key 后保存会替换该平台的 Key；页面不会回显完整密钥。',
        cooldown: '冷却时间（秒）',
        ...
        resume: '恢复',
        paused: '平台已暂停',
        resumed: '平台已恢复，下一笔真实请求将验证可用性',
        pauseFailed: '更新平台暂停状态失败',
        moveUp: '提高优先级',
        moveDown: '降低优先级',
        delete: '删除平台',
        status: { healthy: '健康', degraded: '异常', error: '错误', cooldown: '自动冷却', manual_pause: '手动暂停', half_open: '恢复验证中', disabled: '已禁用' },
      },
      ...
      auditTestImagesHint: '支持上传、拖拽或粘贴图片，最多 16 张，每张不超过 8MB。',
      ...
      sessionBlock: 'API 命中后封禁会话',
      sessionBlockHint: '仅外部审核 API 文本/图片过阈值时写入会话黑名单。关键词拦截不生效。管理员只拦当次请求。缺少会话 ID 不封账号。默认关闭。',
      sessionBlockTTL: '会话封禁时长（秒）',
      sessionBlockTTLHint: '默认 2592000（30 天）。命中后不续期。范围 60–7776000。',
      sessionBlocks: '被封禁会话',
      sessionBlocksHint: '按原始会话 ID 解封。解封用户不会清除会话黑名单。',
      sessionBlockCount: '当前封禁会话：{count} 个',
      sessionBlockSearch: '按会话 ID / 用户 / Key 搜索',
      deleteSessionBlock: '解封会话',
      clearSessionBlocks: '清空全部会话封禁',
      clearSessionBlocksConfirm: '确定清空全部内容审核会话封禁？此操作不会删除审核记录。',
      sessionBlockDeleted: '已解封 {count} 条会话',
      sessionBlockNotFound: '未找到该会话封禁',
      sessionBlockDeleteFailed: '解封会话失败',
      sessionBlocksCleared: '已清空 {count} 条会话封禁',
      sessionBlocksClearFailed: '清空会话封禁失败',
      sessionBlockEmpty: '暂无封禁会话',
      sessionBlockExpires: '过期时间',
      cyberPolicyAutoBan: 'cyber_policy 自动封号',
      cyberPolicyAutoBanHint: '开启后，上游 cyber_policy 命中立即停用。普通用户封账号；管理员只禁用触发该请求的 API Key。默认关闭。',
      ...
      emptyLogs: '暂无审核记录',
      extractionStatus: '内容提取覆盖',
      extractionStatusHint: '每个适用请求都必须归类为提取成功、明确无文本或提取失败；提取失败非零时需要立即排查。',
      extractionAttempted: '尝试提取',
      ...
      keywordModeKeywordOnlyDesc: '文本只用关键词判断，未命中即放行且不调用文本 API；图片仍按全局模式审核。',
      keywordModeKeywordOnlyNotice: '当前为「仅关键词」文本策略：未命中关键词的纯文本请求直接放行；图片仍调用审计 API。',
    ...
    subscriptions: {
      ...
      resetQuotaConfirm: "确定要重置 '{user}' 的 5 小时、每日、每周和每月用量配额吗？用量将归零，窗口从现在重新计算。",
