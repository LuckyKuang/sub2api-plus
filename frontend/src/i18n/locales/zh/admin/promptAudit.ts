export default {
  promptAudit: {
    ...
    pool: {
      ...
      timeoutRange: '支持范围：{min}–{max} 毫秒。', inputLimitRange: '支持范围：{min}–{max} 个 Unicode 字符。', inputLimitBehavior: '当前用户文本超过该值时会继续分片。若希望单消息完整送检，请把上限设为不小于实际字符数，并确认模型上下文足够。多个启用节点同时存在时，以其中最小值为准。',
    ...
    events: {
      title: '审计事件', description: '按身份、IP、入口、风险、Hash 和时间复核事件，详情中可查看已留存的审计内容。', decision: '判定', risk: '风险等级', endpoint: '入口', clientIp: '客户端 IP', filterByIp: '筛选 IP {ip}', groupId: '分组 ID', userId: '用户 ID', apiKeyId: 'API Key ID', keyword: '关键词',
      ...
      selectAll: '选择当前页全部事件', selectEvent: '选择事件 {id}', time: '时间', identity: '用户 / 邮箱 / API Key', user: '用户名', email: '用户邮箱', apiKey: 'API Key 名称', group: '分组', route: '入口 / 模型', result: '判定 / 风险', durations: '排队 / 审计', queueDelay: '排队耗时', auditLatency: '审计耗时', preview: '脱敏预览', empty: '没有符合条件的事件。',
      ...
      promptFull: '已留存审计内容（未脱敏）',
      promptFullHint: '已留存的审计内容随事件入库，仅供管理员复核，并可能在达到存储上限时截断；请按敏感数据妥善处理，切勿外泄。',
      promptTruncatedWarning: '该事件因达到存储上限而被截断，或产生于完整诊断信息可用之前；未留存部分无法恢复。',
      guardReturn: '模型审计返回',
      guardReturnHint: '展示 Guard 归一化后的结构化结果（判定、分类、分数与脱敏证据），不含原始响应体。',
      riskSummaries: '风险摘要',
      evidence: '脱敏证据',
      score: '分数',
      categories: '分类', model: '模型', stage: '请求阶段', executionMode: '执行模式', promptSize: '送检规模', promptSizeValue: '{chars} 字符 · {messages} 条消息', contentStatus: '留存状态', contentComplete: '上限内完整留存', contentTruncated: '已截断', noRisks: '本事件没有派生风险摘要。',
      requestId: 'Request ID', promptHash: 'Prompt SHA-256',
      technical: {
        scanner: '扫描器', policy: '策略', guardEndpoint: 'Guard 节点', config: '配置版本',
        chunks: '分片数', matchedChunk: '命中分片', inputLimit: '单片字符上限', latency: '审计耗时', protocol: '协议',
      },
