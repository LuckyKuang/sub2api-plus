export default {
    settings: {
      ...
      outboundIdentity: {
        ...
        effectiveAccount: '账号最终生效身份',
        client: '客户端标识', version: '客户端版本', source: '生效来源',
        headers: '出站身份请求头', advanced: '高级身份设置',
        inherit: '继承全局默认', builtin: '使用默认映射',
        inheritHint: '留空时使用项目默认。版本号与 UA 中的客户端版本配套生成，SDK 和协议版本保持各自定义。',
        defaults: '账号默认身份映射',
      ...
      features: {
        ...
        modelPlaza: {
		  ...
		  publicExposureWarning: '允许匿名访问会公开非专属分组名称、模型可用性、价格与倍率。专属分组仍不会向匿名访客展示。',
        ...
        riskControl: {
          ...
          disconnectBan: '连续取消请求自动封禁',
          disconnectBanHint: '普通用户在上游已接受后连续取消请求达到阈值时禁用账号；管理员不受影响，任一有效成功请求都会清零连续次数。',
          disconnectBanThreshold: '连续取消阈值',
          disconnectBanThresholdHint: '可配置范围：1-1000，默认 10。',
      ...
      ipAccessControl: {
        ...
        enabled: '启用全局 IP 访问控制',
        enabledHint: '默认关闭。必须先打开本开关，再在 IP 页打开「启用全局 IP 拦截」，两层都开才会拦截。关闭后安全审计入口隐藏，已有规则和 IP 页开关会保留但不会生效。升级后旧站也会先停拦，需要重新打开本开关。',
        manage: '配置 IP 访问控制'
      },
      panelRateLimit: {
        title: '面板接口限流',
      ...
      tencentCaptcha: {
        ...
        regionCn: '中国站',
        regionIntl: '国际站',
        regionHint: '决定前端脚本与服务端接入点。中国站与国际站是两套独立账号体系，需与验证码控制台所属站点一致。',
      ...
      linuxdo: {
        ...
        description: '配置 LinuxDo Connect OAuth，用于 Sub2API Plus 用户登录',
      ...
      dingtalk: {
        title: '钉钉登录',
        description: '配置钉钉 OAuth，用于 Sub2API Plus 用户登录',
      ...
      defaults: {
        ...
        asyncImageUserImagesPerMinute: '每用户每 60 秒最大异步图像张数',
        asyncImageUserImagesPerMinuteHint: '生成和编辑任务共用所有 API Key 与分组额度，按请求的输出张数计算；0 = 不限制。保存后立即生效。',
      ...
      scheduling: {
        ...
        accountSchedulingThresholdsDescription: '当账号当前原生用量窗口（OpenAI Codex/Anthropic 会话，或 Grok 请求/Token 利用率）达到该百分比时，Sub2API Plus 会临时将其移出调度，直到窗口重置。填 100 表示禁用。',
      ...
      gatewayForwarding: {
        ...
        openaiCodexVersionEffective: '当前生效：{version}（{source}）',
        openaiCodexVersionSourceOverride: '手填',
        openaiCodexVersionSourceSynced: '同步',
        openaiCodexVersionSourceCompiled: '内置',
        openaiCodexVersionSyncedValue: '当前同步到：{version}',
        openaiCodexVersionCheckedAt: '上次检查：{time}',
        openaiCodexVersionNeverChecked: '尚未同步',
        openaiCodexVersionSyncNow: '立即同步',
        openaiCodexVersionSyncing: '正在同步…',
        openaiCodexVersionSyncSuccess: '已同步到 {version}',
        openaiCodexVersionSyncUnchanged: '已检查，仍是 {version}',
        openaiCodexVersionSyncFailed: '同步失败：{error}',
        ...
        codexAllowAppServerDesc:
          '放行内嵌 Codex 引擎、经 app-server 协议接入的第三方客户端（如 Claude Code 的 codex 插件）。默认关闭；开启后此类客户端通过引擎指纹门即放行，关闭则仅放行官方客户端与白名单。',
      ...
      payment: {
        ...
        easypayCustomMethodsHint: '添加当前易支付服务商额外支持的支付方式。支付方式会记录到 Sub2API Plus 订单中，上游 type 会作为易支付 type 参数提交。',
      ...
      openaiFastPolicy: {
        ...
        scopeBedrock: '仅 Bedrock 账号',
        userIds: '指定用户',
        userIdsHint: '输入任意邮箱关键词进行模糊搜索。留空表示对全部 Sub2API Plus 用户生效；选中用户的 API Key 请求优先匹配用户规则。',
