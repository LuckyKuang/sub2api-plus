export default {
    ...
    backup: {
      ...
      s3: {
        ...
        bucket: '存储桶',
        prefix: 'Key 前缀',
        appendDatePath: '添加日期路径',
        prefixPreview: '实际前缀：{prefix}',
        accessKeyId: 'Access Key ID',
        secretAccessKey: 'Secret Access Key',
        secretHelp: '保存新的 Secret 前，服务必须已配置固定加密密钥。只测连接，或 Secret 留空沿用已保存的值，不必改这项。',
        secretGuideLink: '查看配置步骤',
      ...
      encryptionKeyGuide: {
        title: '配置固定加密密钥',
        intro: '保存新的 S3 Secret Access Key（备份或异步生图）前，进程必须已有一把固定加密密钥。没配时每次启动会临时生成，重启后库里的密文解不开，保存会被拒绝。',
        whenNeeded: '只点「测试连接」，或 Secret 留空沿用已保存的值，不必改这项。',
        generateTitle: '生成密钥（只做一次）',
        generateHint: '输出应为 64 位十六进制。把结果抄进配置文件，不要写进每次启动都会重新执行的脚本。',
        ...
        dockerWhere: '改 deploy/.env 里的 TOTP_ENCRYPTION_KEY。',
        dockerStep1: '进入部署目录（常见是仓库里的 deploy/）。',
        dockerStep2: '先检查是否已有密钥：grep \'^TOTP_ENCRYPTION_KEY=\' .env',
        dockerStep3: 'docker-deploy.sh 装过的一般已经有值。已有 64 位 hex 就不要覆盖，直接按下面重建容器后回去保存 Secret。',
        dockerStep4: '没有则执行：TOTP_ENCRYPTION_KEY=$(openssl rand -hex 32) 然后 echo "TOTP_ENCRYPTION_KEY=$TOTP_ENCRYPTION_KEY" >> .env',
        dockerStep5: '已有一行但是空的，改成赋值，不要再追加第二行。',
        ...
        binaryWhere: '改 /opt/sub2api/config.yaml 的 totp.encryption_key。官方 install.sh 和设置向导写的就是这个文件。',
        binaryStep1: '确认服务存在：systemctl status sub2api',
        binaryStep2: '先看有没有现成的密钥：sudo grep -n encryption_key /opt/sub2api/config.yaml',
        binaryStep3: '已是 64 位 hex 就回去保存 Secret，不要再生成。',
        binaryStep4: '没有则生成一次：openssl rand -hex 32，复制输出。',
        binaryStep5: '编辑配置：sudo nano /opt/sub2api/config.yaml',
        binaryYamlHint: '没有 totp: 时在文件末尾追加；已有 totp: 时只补/改正 encryption_key，不要改同段其它项。',
        binaryYaml: 'totp:\n  encryption_key: "这里粘贴64位hex"',
        ...
        binaryLogHint: '日志中不应再出现 TOTP encryption key auto-generated。',
        ...
        standaloneStep1: '写入与官方二进制相同的 totp.encryption_key。',
        standaloneStep2: '停掉旧进程再启动新进程。只刷新网页不会加载这把密钥。',
        afterTitle: '重启后再保存',
        afterHint: '回到本页，重新填写 Secret Access Key 并保存。网页里保存设置不会加载这把密钥。',
        warningsTitle: '不要做这些事',
        warningExisting: '已有 TOTP 用户或已经存过 S3 Secret 时，不要更换正在使用的这把密钥，否则旧数据解不开。',
        warningOnce: 'openssl rand 只跑一次。不要把生成命令写进每次启动的脚本里现算。',
        warningNotMinio: '这把密钥不是 MinIO/S3 的 Access Key。对象存储密钥可以换；加密密钥必须长期固定。',
        warningWrongFile: 'Docker 改 deploy/.env，官方二进制改 /opt/sub2api/config.yaml，不要改到别的配置文件。'
      }
    },
    ...
    groups: {
      ...
      subscription: {
        ...
        quotaFollowReset: {
          ...
          hint: '只能选择已绑定到本分组的 OpenAI OAuth 账号。首次开启、重新开启或更换来源后，先获取当前官方周窗口建立新基线，不立即重置。随后确认自然到期或官方提前重置时，本分组只重置一次。后台定期刷新来源；无需捕获到用量为零，不补算或回填历史消费，且不影响账号路由。',
          ...
          waitingBaseline: '正在获取本次开启后的官方周窗口基线；不会沿用旧基线或立即重置额度。',
      ...
      profitControl: {
        enable: '启用利润控制',
        enabledHint: '调度时仅允许"账号倍率 ≤ 请求实际下游倍率 ×（1 − 最低毛利率 − 安全缓冲）"的账号进入候选池；账号倍率由管理员手工维护，既有排序、粘性与熔断在合格账号间照常工作。图片/视频调度暂不参与。',
