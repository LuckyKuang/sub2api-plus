...
Docker image tag is `ghcr.io/luckykuang/sub2api-plus:0.1.164-custom.005`.
...
- Release archives must retain the `+` suffix, for example
  `sub2api_0.1.164+custom.005_linux_amd64.tar.gz`.
- GHCR image tags use the Docker-safe `-` separator, for example
  `ghcr.io/luckykuang/sub2api-plus:0.1.164-custom.005`.
...
6. Run backend and frontend checks, then publish the matching GitHub Release
   tag, for example `v0.1.164+custom.005`.
